...
class AdministradorFiltros:
    ...
    def __init__(self, parent):
        ...
        if hasattr(parent, "destroyed"):
            ...
            logger.debug("Configurado detector de destruição do objeto pai")

    def desconectar_sinais(self):
        try:
            logger.debug("Desconectando sinais do AdministradorFiltros")
                    ...
                    try:
                        if hasattr(sinal, 'disconnect'):
                            try:
                                ...
                                logger.debug("Sinal desconectado com sucesso")

                            except RuntimeError:
                                logger.debug("Sinal já desconectado ou inválido")

                            except TypeError:
                                logger.debug("Tipo inválido durante desconexão")

                        else:
                            logger.debug("O objeto não possui método disconnect")

                    except Exception as e:
                        logger.debug(f"Erro ao verificar/desconectar sinal: {e}")
            ...
            for widget in QApplication.topLevelWidgets():
                if hasattr(widget, 'tabela_dados'):
                    try:
                        ...
                        logger.debug("Tabela de dados da janela principal não está disponível")
                        break
    ...
    def atualizar_contagem_apos_idioma(self, idioma=None):
        try:
            logger.debug(f"Atualizando contagem após mudança de idioma para: {idioma}")

            if self.parent is None:
                logger.debug("Parent não existe mais, ignorando atualização")
                return

            import shiboken6
            if not shiboken6.isValid(self.parent):
                logger.debug("Parent não é mais válido, ignorando atualização")
                return

            if hasattr(self.parent, 'filtroAplicado'):
                try:
                    self.parent.filtroAplicado.emit()
                    logger.debug("Sinal filtroAplicado emitido com sucesso")

                except RuntimeError as re:
                    logger.warning(f"Não foi possível emitir sinal filtroAplicado: {re}")
            ...
            logger.warning(f"Erro ao atualizar após mudança de idioma: {e}", exc_info=True)
                ...
                if hasattr(widget, 'gerenciador_eventos_ui'):
                    try:
                        if hasattr(widget.gerenciador_eventos_ui, 'atualizar_interface'):
                            ...
                            logger.debug(f"Objeto gerenciador_eventos_ui não possui método atualizar_interface")
