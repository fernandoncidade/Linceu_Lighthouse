...
def identificar_tipo_arquivo(caminho, loc, nome_arquivo=None):
    ...
    TIPOS_IMAGEM = {'.jpg', '.jpeg', '.png', '.gif', '.bmp', '.tiff', '.tif', '.psd',
                    '.webp', '.raw', '.heic', '.heif', '.cr2', '.nef', '.arw', '.ico'}
    ...
    TIPOS_CODIGO_FONTE = {'.py','.java', '.cpp', '.c', '.h', '.hpp','.cs', '.js', '.html',
                          ...
                          '.xml', '.yaml', '.yml', '.md', '.rst', '.syms', '.svg',
                          '.bat', '.sh', '.ps1', '.psm1', '.psd1', '.ps1xml', '.pssc', '.psc1'}
    TIPOS_DOCUMENTO = {'.doc', '.docx', '.pdf', '.rtf', '.txt', '.odt', '.wpd', '.pages', '.map', 
                       '.md5', '.dotx', '.docm', '.dotm', '.msg', '.pst', '.ost', '.pub'}
    TIPOS_PLANILHA = {'.xls', '.xlsx', '.xlsm', '.ods', '.csv', '.tsv', '.numbers', '.xltx', '.xltm'}
    TIPOS_APRESENTACAO = {'.ppt', '.pptx', '.odp', '.key', '.potx', '.ppsx'}
    TIPOS_BANCO_DADOS = {'.dat', '.db', '.sqlite', '.mdb', '.accdb', '.sav', '.spss', '.db-journal'}
    TIPOS_EXECUTAVEIS = {'.exe', '.dll', '.bin', '.app', '.apk', '.msi', '.run', '.bat', '.cmd', '.pma', '.pbtxt',
                         '.binarypb', '.tflite', '.fst', '.otf', '.ldb', '.leveldb', '.blob', '.lnk', '.hyb', '.msix'}
    TIPOS_COMPACTADOS = {'.zip', '.rar', '.7z', '.tar', '.gz', '.bz2', '.xz', '.cab', '.pb', '.pb.gz', '.tgz', '.tar.gz'}
