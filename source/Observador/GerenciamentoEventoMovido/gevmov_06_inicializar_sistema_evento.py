...
def _inicializar_sistema_evento(interfaceMonitor):
    try:
        from source.Observador.ob_08_EventoMovido import EventoMovido, EventoBuffer
        if not hasattr(interfaceMonitor, 'processador_evento'):
            ...
            if not hasattr(interfaceMonitor, 'movimentacao_worker'):
                from source.Observador.GerenciamentoEventoMovido.gevmov_01_MovimentacaoWorker import MovimentacaoWorker
