            ...
            try:
                ...
                if info:
                    ...
                    try:
                        ...
                        try:
                            raw_creation = reader.metadata.get('/CreationDate')
                            if raw_creation:
                                metadados['data_criacao_doc'] = parse_pdf_date(raw_creation)

                        except Exception:
                            logger.warning(f"Não foi possível extrair data de criação do PDF {caminho}")
                    ...
                    try:
                        ...
                        try:
                            raw_modification = reader.metadata.get('/ModDate')
                            if raw_modification:
                                metadados['data_mod_doc'] = parse_pdf_date(raw_modification)

                        except Exception:
                            logger.warning(f"Não foi possível extrair data de modificação do PDF {caminho}")
