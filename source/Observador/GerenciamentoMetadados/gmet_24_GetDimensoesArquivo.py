...
def get_dimensoes_arquivo(self, item, loc):
    try:
            ...
            if ext == '.bak' in nome_arquivo or 'backup' in nome_arquivo or 'bkp' in nome_arquivo:
                ...
                try:
                    ...
                    with self.lock_cache:
                        ...
                        self.cache_metadados[caminho]["linhas"] = str(linhas)
                        self.cache_metadados[caminho]["palavras"] = str(palavras)
                        self.cache_metadados[caminho]["caracteres"] = str(caracteres)
                        self.cache_metadados[caminho]["tamanho"] = formata_tamanho(tamanho)
        ...
        logger.error(f"Erro ao obter dimensões do arquivo {caminho}: {e}", exc_info=True)
